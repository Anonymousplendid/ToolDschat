...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    elif "tool" in dataset_name:
        return raw_datasets.ToolMathDataset(output_path, seed, local_rank, dataset_name)
...
class PromptDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.train_phase == 1:
            ...
            return self.prompt_dataset[idx][0]["input_ids"],self.prompt_dataset[idx][0]["attention_mask"], \
                self.pad_token_id, self.prompt_dataset[idx][1]
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    if train_phase == 1:
        for i, tmp_data in enumerate(current_dataset):
            prompt = raw_dataset.get_prompt(tmp_data).strip()
            chosen = raw_dataset.get_chosen(tmp_data).rstrip()
            prompt = prompt.replace("Human:", f"{tokenizer.eos_token}Human:")
            prompt = prompt.lstrip(tokenizer.eos_token)

            chosen = chosen + tokenizer.eos_token
            
            prompt_ids = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(prompt))
            chosen_ids = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(chosen))
            if len(prompt_ids) + len(chosen_ids) > max_seq_len - 1:
                if len(chosen_ids) > max_seq_len - 1:
                    prompt_ids = []
                    chosen_ids = chosen_ids[:max_seq_len - 1]
                else:
                    prompt_ids = prompt_ids[-(max_seq_len - 1 - len(chosen_ids)):]
            prompt_ids = [tokenizer.bos_token_id] + prompt_ids

            input_ids = prompt_ids + chosen_ids
        ...
        for i, tmp_data in enumerate(current_dataset):
            prompt = raw_dataset.get_prompt(tmp_data).strip()
            chosen = raw_dataset.get_chosen(tmp_data).rstrip()
            reject = raw_dataset.get_rejected(tmp_data).rstrip()
            prompt = prompt.replace("Human:", f"{tokenizer.eos_token}Human:")
            prompt = prompt.lstrip(tokenizer.eos_token)
            ...
            prompt_ids = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(prompt))
            chosen_ids = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(chosen))
            reject_ids = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(reject))

            maxlen  = max(len(chosen_ids), len(reject_ids))
            if len(prompt_ids) + maxlen > max_seq_len - 1:
                if maxlen > max_seq_len - 1:
                    prompt_ids = []
                    chosen_ids = chosen_ids[:max_seq_len - 1]
                    reject_ids = reject_ids[:max_seq_len - 1]
                else:
                    prompt_ids = prompt_ids[-(max_seq_len - 1 - maxlen):]
            prompt_ids = [tokenizer.bos_token_id] + prompt_ids
            ...
            input_ids = prompt_ids + reject_ids
            attention_mask = [1] * len(input_ids)
...
class ToolPromptDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.prompt_dataset[idx]["prompt"], self.prompt_dataset[idx]["prompt_att_mask"], \
                self.pad_token_id, self.prompt_dataset[idx]["labels"], self.prompt_dataset[idx]["result_prompt"]


def create_tool_dataset(current_dataset, raw_dataset, train_phase, tokenizer, end_of_conversation_token,
                                            max_seq_len):
    if train_phase != 3:
        return NotImplementedError
    assert raw_dataset.selfmade and raw_dataset.tool
    too_long_filter = 0
    prompt_dataset = []
    STOP_INDEX = 100000
    for i, tmp_data in tqdm(enumerate(current_dataset)):
        if i >= STOP_INDEX:
            break
        tool_prompt = raw_dataset.get_tool_prompt(tmp_data)
        if not tool_prompt:
            continue
        result_prompt = raw_dataset.get_result_prompt()
        if tool_prompt and result_prompt:
            tool_prompt_token = tokenizer(tool_prompt, return_tensors = "pt")
            
            if tool_prompt_token["input_ids"].size()[-1] <= max_seq_len:
                all_prompts = dict()
                for key_word in ["input_ids", "attention_mask"]:
                    tool_prompt_token[key_word] = tool_prompt_token[key_word].squeeze(0).flip(0)
                    
                all_prompts["prompt"] = tool_prompt_token["input_ids"]
                all_prompts["prompt_att_mask"] = tool_prompt_token["attention_mask"]
                import re
                match = re.search(r"b'(.*?)\\n'", tmp_data["answer"])
                all_prompts["labels"] = "{:.2f}".format(eval(match.group(1))) 
                all_prompts["result_prompt"] = result_prompt
                prompt_dataset.append(all_prompts)
            else:
                too_long_filter += 1
    print(f'Creating dataset {raw_dataset.dataset_name_clean} '
              f'for {train_phase=} size={len(prompt_dataset)} {too_long_filter=}')
    return ToolPromptDataset(prompt_dataset, tokenizer.pad_token_id, train_phase)      
...
def create_dataset(local_rank, dataset_name, data_split, output_path,
    ...
    if not getattr(raw_dataset, "selfmade", False):
        ...
        train_dataset = create_tool_dataset(train_dataset, raw_dataset,
                                            train_phase, tokenizer,
                                            end_of_conversation_token,
                                            max_seq_len)
        eval_dataset = create_tool_dataset(eval_dataset, raw_dataset,
                                                 train_phase, tokenizer,
                                                 end_of_conversation_token,
                                                 max_seq_len)
...
class DataCollatorTool:
    ...
    def __call__(self, data):
        ...
        batch["prompt"] = batch["prompt"].flip(1)
        batch["prompt_att_mask"] = batch["prompt_att_mask"].flip(1)
        batch["labels"] = [f[3] for f in data]
        batch["result_prompt"] = [f[4] for f in data]
        return batch
