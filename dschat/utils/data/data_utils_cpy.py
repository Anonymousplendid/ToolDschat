...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    if train_phase == 1:
        ...
        filtered = 0
        for i, tmp_data in enumerate(tqdm(current_dataset)):
            if False:
                # tokenize the text
                prompt = raw_dataset.get_prompt(tmp_data).strip()
                if prompt is not None:
                    ...
                    if prompt_token["input_ids"].size()[-1] <= max_seq_len:
                        ...
                        filtered += 1
            elif "qwen" in tokenizer.init_kwargs["name_or_path"].lower():
                prompt, target = raw_dataset.get_rlreward(tmp_data)
                prompt_token = tokenizer(prompt, return_tensors="pt",)
                if prompt_token["input_ids"].size()[-1] <= max_seq_len:
                    for key_word in ["input_ids", "attention_mask"]:
                        prompt_token[key_word] = prompt_token[
                            key_word].squeeze(0).flip(0)
                        prompt_dataset.append((prompt_token, target))
                else:
                    filtered += 1
            else:
