...
from dschat.utils.tools.tooluse import generate_tool_response
...
class DeepSpeedPPOTrainer():
    ...
    def generate_tool_use(self, prompts, mask, step):
        self.eval()
    ...
    def generate_experience(self, prompts, mask, step, targets=None, result_prompt=None):
        ...
        seq = exp1["input_ids"]
        answers = self.tokenizer.batch_decode(seq, skip_special_tokens=True)
        new_prompts = []
        prompt = result_prompt
        
        
        import re
        pattern = r"\[TASK\]:([\w\d\s\n,.!?\(\):\+\-\*\/]+)\nOutput: ([\w\d\s,!?\(\)\[\]\+\-\*\/]+)."
        for i in range(len(answers)):
            try:
                match = re.search(pattern, answers[i])
                api_call = generate_tool_response(match.group(2))
                new_prompt = prompt[i] + match.group(1) + " " + api_call + "\nOutput: "
            except:
        ...
        generate_end = time.time()
        self.generate_time = generate_end - generate_start
        exp1["rewards"] = exp2["rewards"]

        self.last_generated_experience = (exp1, exp2)
        return exp1, exp2
