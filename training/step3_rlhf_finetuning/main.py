...
from dschat.utils.data.data_utils import create_prompt_dataset, MiniDataset, DataCollatorRLHF, get_unsupervised_data, DataCollatorTool
...
from dschat.utils.module.lora import convert_lora_to_linear_layer, unfuse_lora, fuse_lora
...
def parse_args():
    ...
    args.enable_tensorboard = args.enable_tensorboard and args.local_rank == 0
...
def create_datasets(args, tokenizer, train_phase=3):
    ...
    data_collator = DataCollatorTool(args.max_prompt_seq_len,
...
def main():
    ...
    if args.actor_gradient_checkpointing:
        trainer.actor_model.enable_input_require_grads()
    if args.critic_gradient_checkpointing:
        trainer.critic_model.rwtransformer.enable_input_require_grads()
    ...
    #             ) == 0:
    #     wandb.init(project="sakura_research",
    #            name="rlaif_qwen_jh_v1",
    #            job_type="training")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            if batch_prompt["labels"] is not None:
                out = trainer.generate_experience(batch_prompt['prompt'],
                                                    batch_prompt['prompt_att_mask'],
                                                    step,
                                                    batch_prompt['labels'],
                                                    batch_prompt["result_prompt"])
            else:
                out = trainer.generate_experience(batch_prompt['prompt'],
                                                    batch_prompt['prompt_att_mask'],
                                                    step)
            ...
            if exp_dataset is not None:
                ...
                if args.enable_tensorboard and torch.distributed.get_rank(
                    ...
                    writer.add_scalar('train_ppo/reward',
                                      average_reward / inner_iter,
                                      global_step=step)
                    writer.add_scalar('train_ppo/actor_loss',
                                      actor_loss.item(),
                                      global_step=step)
                    writer.add_scalar('train_ppo/actor_loss_sum',
                                      actor_loss_sum / inner_iter,
                                      global_step=step)
                    writer.add_scalar('train_ppo/critic_loss',
                                      critic_loss.item(),
                                      global_step=step)
                    writer.add_scalar('train_ppo/critic_loss_sum',
                    ...
                    if trainer.reward_scaling:
                        writer.add_scalar("train_ppo/reward_mean",
                                          trainer.adv_mean.item(),
                                          global_step=step)
                        writer.add_scalar("train_ppo/reward_adv",
                                            trainer.adv_var.item(),
                                            global_step=step)
                        writer.add_scalar("train_ppo/reward_cnt",
    ...
    if args.output_dir is not None:
        ...
        rlhf_engine.actor = fuse_lora(rlhf_engine.actor)
        rlhf_engine.critic = fuse_lora(rlhf_engine.critic)
        if args.enable_ema:
            rlhf_engine.actor_ema = fuse_lora(rlhf_engine.actor_ema)
