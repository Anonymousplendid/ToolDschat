...
from dschat.utils.model.reward_model import ToolReward
import os, json
"""
TODOs:
...
class DeepSpeedRLHFEngine():
    ...
    def __init__(self, actor_model_name_or_path, critic_model_name_or_path,
        ...
        # model_json = os.path.join(actor_model_name_or_path, "config.json")
        # assert os.path.exists(model_json), f"model json {model_json} does not exist"
        # model_json_file = json.load(open(model_json, "r"))
        # model_name = model_json_file["_name_or_path"]
        # assert model_name is not None, f"model name is None in {model_json}"
        ...
        for n, p in self.actor.named_parameters():
            ...
            if p.requires_grad:
                param_cnt += p.numel()
        print("actor需要训练{}参数".format(param_cnt))
        param_cnt = 0
        for n, p in self.critic.named_parameters():
            ...
            if p.requires_grad:
                param_cnt += p.numel()
        print("critic需要训练{}参数".format(param_cnt))
    ...
    def _init_actor(self, actor_model_name_or_path):
        ...
        if self.args.actor_lora_dim > 0:
            actor_model = convert_linear_layer_to_lora(
                ...
                self.args.actor_lora_dim, self.args.lora_alpha,)
        ...
        # LR Scheduler
        if self.args.actor_sleep > 0:
            lr_scheduler = get_scheduler(
                # name="cosine_with_sleep",
                name = "constant_with_warmup",
                optimizer=optim,
                num_warmup_steps=self.args.num_warmup_steps,
                num_training_steps=self.num_total_iters,
                # sleep_steps=self.args.actor_sleep,
            )
        else:
            lr_scheduler = get_scheduler(
    ...
    def _init_critic(self, critic_model_name_or_path):
        ...
        if self.args.critic_lora_dim > 0:
            critic_model = convert_linear_layer_to_lora(
                ...
                self.args.critic_lora_dim, self.args.lora_alpha,)
            if self.args.only_optimize_lora:
                force_optimize_params = ["v_head.weight"]
                critic_model = only_optimize_lora_parameters(critic_model, force_optimize_params)
