...
ACTOR_MODEL_PATH=/aiarena/gpfs/models/qwen2-4b
...
klclip=2
POSITIONAL=()
OUTPUT=${OUTPUT}/lora${Lora_Lr}_promptlen${promptlen}_answerlen${answerlen}_loradim${loradim}_batchsize_${batchsize}_asleep${actorsleep}
...
deepspeed --include localhost:0,1,2,3,4,5,6,7 --master_port 12346 main.py \
   --data_path /aiarena/gpfs/data/tools/math_dataset/arithmetic__mixed \
   --data_split 2,4,4 \
   --data_output_path /aiarena/gpfs/tmp \
   ...
   --lora_alpha $loradim \
   --only_optimize_lora \
   --actor_lora_module_name "self_attn" \
   --critic_lora_module_name "self_attn" \
   --output_dir $OUTPUT \
   --enable_tensorboard \
   ...
   --kl_clip ${klclip} \
   --dtype bf16 \
   --actor_sleep ${actorsleep} > $OUTPUT/training.log 2>&1
